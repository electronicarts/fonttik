...
namespace tin {
	...
	void Image::saveResultsOutlines(std::vector<std::pair<int, std::vector<ResultBox>>>& results, fs::path path, bool saveNumbers) {
		cv::Mat highlights = imageMatrix.clone();

		for (ResultBox& box : results.back().second) {
